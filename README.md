...
React framework that allows developers to write HTML-like markup directly within their 
JavaScript code. It is not valid JavaScript that browsers can read directly, so it must be
transpiled into regular JavaScript before it can run. </br>
...
- npm run lint : Is the command that runs eslint.  EsLint checks our project file, and give us the current list of EsLint errors. </br>
E.G you can go to App.jsx and make a typo error on App to Ap , to see the error when you rerun the command.
